import db from '../db/index.js'

export async function calcHoldings(userId) {
  const { rows } = await db.query(
    'SELECT * FROM transactions WHERE user_id = $1 ORDER BY date ASC',
    [userId]
  )
  const map = {}
  for (const t of rows) {
    if (!map[t.code]) {
      map[t.code] = { code: t.code, name: t.name, category: t.category || '', shares: 0, total_cost: 0 }
    }
    if (t.type === 'buy') {
      map[t.code].total_cost += Number(t.shares) * Number(t.price) + Number(t.fee || 0)
      map[t.code].shares += Number(t.shares)
    } else {
      const ratio = Number(t.shares) / map[t.code].shares
      map[t.code].total_cost -= map[t.code].total_cost * ratio
      map[t.code].shares -= Number(t.shares)
    }
  }
  return Object.values(map).filter(h => h.shares > 0)
}

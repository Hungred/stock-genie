...
<template>
  <div>
    <h1 class="text-2xl font-bold text-gray-800 mb-6">配息記錄</h1>

    <!-- 統計 -->
    <div class="grid grid-cols-2 gap-4 mb-8">
      <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <p class="text-sm text-gray-500 mb-1">累計領取配息</p>
        <p class="text-2xl font-bold text-blue-600">{{ totalDividend.toLocaleString() }}</p>
        <p class="text-xs text-gray-400">元</p>
      </div>
      <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <p class="text-sm text-gray-500 mb-1">近期配息提醒</p>
        <p class="text-2xl font-bold text-orange-500">{{ upcomingDividends.length }}</p>
        <p class="text-xs text-gray-400">支股票本月/下月配息</p>
      </div>
    </div>

    <!-- 近期配息提醒 -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6" v-if="upcomingDividends.length">
      ...
      <div class="p-4 flex flex-wrap gap-3">
        ...
        >
          <span class="font-medium text-gray-700">{{ h.code }}</span>
          <span class="text-sm text-gray-500">{{ h.name }}</span>
